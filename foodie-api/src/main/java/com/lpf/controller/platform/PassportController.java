...
import static com.lpf.consts.OtherConsts.FOODIE_SHOPCART;
...
public class PassportController {
    ...
    @PostMapping("/regist")
    public ResultJSONResult regist(@RequestBody UserBO userBO, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (password.length() < 6) {
            return ResultJSONResult.errorMsg("密码长度不能少于6");
        }

        // 3. 判断两次密码是否一致
        if (!password.equals(confirmPwd)) {
            return ResultJSONResult.errorMsg("两次密码输入不一致");
        }
    ...
    public ResultJSONResult login(@RequestBody UserBO userBO,
        ...
        return ResultJSONResult.ok(userResult);
    }
    ...
    @PostMapping("/logout")
    public ResultJSONResult logout(@RequestParam String userId, HttpServletRequest request,  HttpServletResponse response) {
